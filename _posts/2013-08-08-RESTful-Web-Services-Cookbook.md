...
- 400(Bad Request)，当服务器由于语法错误无法解读请求时，返回该错误码。
- 401(Unauthorized)，当客户端无权访问资源，但在身份验证后可以获得访问权限时返回该错误码。返回该错误码时，应该包含一个带有身份验证方法的WWW-Authenticate头。通常使用的方法是Basic和Digest。
- 403(Forbidden)，当服务器不让客户端获得资源的访问权限，就算通过身份验证也没用时，返回该错误码。
- 404(Not Found)，当没有找到资源时返回该错误码。如果可能的话，在消息体中说明原因。
- 405(Not Allowed)，当资源不允许使用某个HTTP方法时返回该错误码。返回一个Allow头，其中带有该资源的有效HTTP方法。
- 406(Not Acceptable)
- 409(Conflict)，当请求与资源的当前状态有冲突时返回该错误码。
- 410(Gone)，资源以前存在，但今后不会再存在时，返回该错误码。除非记录了被删除的资源，否则不能返回这个错误码。
- 412(Precondition Failed)
- 413(Request Entity Too Large)，当POST或PUT请求的消息体过大时返回该错误码。
- 415(Unsupported Media Type)，当客户端用一种服务器不理解的格式来发送消息体时返回该错误。

由服务器端导致的错误：5xx

- 500(Internal Server Error)，由于某些实现上的问题，您的代码在服务器端失败时返回错误码是最好的选择。
- 503(Service Unavailable)，服务器在某个特定间隔或一段不确定的时间内无法完成请求时，返回该错误码。两个常见场合是后端服务器失败（例如数据库连接失败）或者是客户端请求达到了某个服务器设定的频率上限。

**针对错误的消息正文**

对于所有的错误，都要在错误响应中包含一段正文，当HTTP方法是HEAD时除外。其中包含以下部分或全部内容：
...
在实现客户端时，有两类错误是客户端需要处理的。第一类是网络层面的失败，第二类是服务器返回的HTTP错误。前者是编程库来处理，后者是特定与应用程序的，要求显示地编写代码来做处理。

明确地处理各种错误码可以让客户端变得更坚固。HTTP状态码是可扩展的，服务器可以引入新状态码。如果客户端不理解Xmn状态码，那么应该将其视为X00。不要把HTTP错误视为I/O或网络异常，把它们当作一等应用程序对象。

##Chapter 4 设计URI##

URI是跨越Web的资源描述符，一个URI由以下内容组成——协议（例如http和https）、主机（例如www.example.org）、端口号，后面紧跟一段或多段路径（例如/user/1234），还有查询字符串。

###4.1 如何设计URI###

针对本地化、分布式、强化多种监控以及安全策略等方面的需求，可以使用域以及子域对资源进行合理的分组或划分。
...
从逻辑上将URI分成域和子域可以为服务器管理提供很多操作方面的优势。

	http://www.example.org/book/1234
	http://api.example.org/book/1234

在这个例子中，服务器提供了两个子域，一个针对浏览器，另一个针对自定义的客户端。这样的划分可以让服务器针对HTML和非HTML表述分配不同的硬件，应用不同的路由，监控或安全策略。

应当尽量避免在URI中使用大写字符。

##Chapter 5 查询##

查询通常涉及三个组成部分，即过滤（filtering）、排序（sorting）、投影（projection）。
...
将查询响应的表述设计为集合资源。设置合理的过期缓存头。如果查询没有匹配到任何资源，返回一个空集合。

###5.3 如何支持有大量输入的查询请求###

使用HTTP POST来支持大查询。使用POST来处理查询削弱了HTTP的统一接口，根据定义，GET才是用于安全、幂等地获取信息的。

###5.4 如何存储查询###

存储查询可以让那些使用POST方式发送的查询变得可以缓存。当客户端用POST发起一个查询请求时，创建一个新资源，它的状态中包含查询条件。返回一个带Location头的响应码201（Created），Location指向创建的资源。实现一个针对新资源的GET请求，返回查询结果。如果同一客户端或另一个客户端用POST发起了相同查询请求，找到匹配该请求的资源，客户端被重定向到该资源的URI上。

存储查询弥补了使用POST方式处理查询的一些局限。缺点是不得不将查询永久保存为一个资源。